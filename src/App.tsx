...
import {
  ...
  countMigos,
  getBestMove,
  getLastSearchStats, // ← TAMBAHKAN INI
  type SearchStats,
...
import NodeStatsPanel from "./components/NodeStatsPanel";
...
export default function App() {
  ...
  // Game coordinates
  const [lastPlacedCell, setLastPlacedCell] = useState<[number, number] | null>(
    null,
  );
  ...
  const [nodeStats, setNodeStats] = useState<SearchStats | null>(null);
  ...
  useEffect(() => {
    ...
    if (phase !== "playing" || winner || !isAiTurn) return;

    setIsAiThinking(true);
    setDragonHint(null);

    const thinkingDelay =
      settings.aiDifficulty === "baby"
        ? 600
        : settings.aiDifficulty === "novice"
          ? 800
    ...
    const timer = setTimeout(() => {
      ...
      // ↓ Ambil node stats setelah getBestMove selesai
  ...
  const processMove = (r: number, c: number, activePlayer: Player) => {
    try {
      ...
      const { newBoard, yugoCreated, affectedCells, yugoCell } = executeMove(
        board,
        r,
        c,
        activePlayer,
  ...
  const handleGetAiHint = () => {
    ...
    const randomWisdom =
      wisdomPills[Math.floor(Math.random() * wisdomPills.length)];
  ...
  return (
    <div className="min-h-screen bg-[#11241a] text-[#fbf9f4] flex flex-col items-center justify-between p-4 selection:bg-red-700 selection:text-white font-sans overflow-x-hidden relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight font-serif text-[#faf8f2]">
              MigoYugo{" "}
              <span className="text-[#cbb27a] font-light text-xl sm:text-2xl">
                Mahjong
              </span>
            </h1>
            <p className="text-xs text-[#faf8f2]/60 font-serif leading-none mt-0.5">
              Abstract Strategy of Complete Information • Est. 2026
            </p>
          </div>
        </div>
          ...
          {phase === "menu" && (
            ...
            >
              <div className="text-center space-y-2">
                <span className="text-xs font-serif font-bold tracking-widest text-[#8b261a] uppercase">
                  Pilih Arena & Pendekar
                </span>
                <h2 className="text-3xl font-bold font-serif text-[#8b261a]">
                  Mata Angin Naga
                </h2>
                <p className="text-xs text-gray-500 font-serif">
                  Konfigurasikan gaya bermain Anda untuk memulai pertempuran
                  pikiran MigoYugo.
                </p>
              </div>
              ...
              <div className="space-y-2.5">
                <label className="text-xs font-bold text-gray-700 uppercase tracking-wider font-serif">
                  Mode Permainan
                </label>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <div>
                      <div className="text-xs font-bold font-serif leading-none">
                        Lokal 2-Pemain
                      </div>
                      <span className="text-[10px] text-gray-500 leading-none">
                        Pass-and-Play bergantian
                      </span>
                    </div>
                  </button>
                ...
                >
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        <span className="text-red-700 text-lg">中</span>{" "}
                        Pendekar Merah (Pertama)
                      </button>
                      <button
                        onClick={() => {
                          sounds.playClick();
                      ...
                      >
                        <span className="text-emerald-700 text-lg">發</span>{" "}
                        Pendekar Hijau (Kedua)
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-gray-700 uppercase tracking-wider font-serif">
                      Tingkat Kesaktian Komputer
                    </label>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      {(
                        ["baby", "novice", "master", "grandmaster"] as const
                      ).map((diff) => {
                        const diffLabels = {
                          baby: "Baby Naga",
                          novice: "Novice Monk",
                          master: "Mahjong Master",
              ...
              <div className="space-y-3.5 border-t border-gray-200/65 pt-3">
                <div className="flex flex-col gap-1">
                  <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider font-serif">
                    Pendekar Merah (Mulai Pertama)
                  </label>
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((prev) => ({
                          ...prev,
                          p1Name: e.target.value || "Pemain Merah",
                        }))
                      }
                  ...
                  <div className="relative flex items-center">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setSettings((prev) => ({
                          ...prev,
                          p2Name: e.target.value || "Pemain Hijau",
                        }))
                      }
                      className="w-full pl-9 pr-3 py-2 bg-white border border-gray-300 rounded-lg text-sm text-gray-800 font-bold focus:outline-none focus:ring-2 focus:ring-[#8b261a] focus:border-[#8b261a] disabled:bg-gray-100 disabled:text-gray-500"
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] text-gray-500">
                      Kekalahan instan bila waktu habis
                    </span>
                  </div>
                  <button
                    id="toggle-clock-btn"
                    onClick={() => {
                      ...
                      setSettings((prev) => ({
                        ...prev,
                        clockEnabled: !prev.clockEnabled,
                      }));
                    }}
            ...
            >
                  ...
                  >
                    ...
                    <div>
                      <h4 className="text-xs font-serif font-bold text-amber-900 flex items-center gap-1.5">
                        <span>🐉</span> Bisikan Taktis Naga Langit (Saran:{" "}
                        {String.fromCharCode(65 + dragonHint.coordinate[1])}
                        {8 - dragonHint.coordinate[0]})
                      </h4>
              ...
              <div className="flex flex-col items-center gap-4 w-full max-w-lg lg:max-w-md">
                ...
                <div className="w-full bg-[#162f22] p-3 rounded-lg border border-emerald-800/40 select-none text-[10px] sm:text-xs">
                  <span className="font-bold font-serif text-[#cbb27a] block mb-1">
                    LOG PERANG MAHJONG:
                  </span>
                    ...
                    {lastPlacedCell ? (
                      <div className="flex justify-between">
                        ...
                        <span className="text-[#cbb27a] font-bold">
                          {turn === "black" ? p1DisplayName : p2DisplayName}{" "}
                          meletakkan di{" "}
                          {String.fromCharCode(65 + lastPlacedCell[1])}
                          {8 - lastPlacedCell[0]}
                        </span>
                      </div>
                    ) : (
                      <div className="text-gray-400 font-serif italic text-center py-2">
                        Permainan dimulai. Angin Timur bertiup kencang...
                      </div>
                    )}
                    {stats.whiteYugos > 0 && (
                ...
                <NodeStatsPanel stats={nodeStats} isThinking={isAiThinking} />
              </div>
            </motion.div>
          )}
          ...
          {phase === "gameover" && (
            ...
            >
              ...
              <div className="text-center space-y-2">
                ...
                <span className="text-xs font-serif font-bold tracking-widest text-[#8b261a] uppercase leading-none">
                  Puncak Pertempuran Pikiran
                </span>
                <h2 className="text-3xl font-bold font-serif text-[#8b261a] leading-tight">
                  KITAB KEMENANGAN
                </h2>
              </div>
              ...
              <div className="bg-[#ede7d5] p-5 rounded-xl border border-amber-900/10 text-center space-y-2">
                {winner === "draw" ? (
                  <div className="space-y-1">
                    <h3 className="text-xl font-bold font-serif text-[#8b261a]">
                      KEDUA PENDEKAR SEIMBANG (SERI)
                    </h3>
                    <p className="text-xs text-gray-600 font-serif">
                      Kekuatan batin keduanya sama tangguhnya di bawah kolong
                      langit.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <span className="text-[10px] font-mono text-[#b45309] uppercase tracking-wider font-bold">
                      PEMULIA SEJATI
                    </span>
                ...
                <div className="mt-3.5 inline-block bg-[#8b261a] text-[#faf8f2] text-[10px] sm:text-xs font-serif font-bold uppercase tracking-wider px-3 py-1 rounded-full border border-yellow-300">
                  {winType === "igo" &&
                    "🏅 MENANG MUTLAK - IGO (4 YUGO SEGARIS)"}
                  {winType === "wego" &&
                    "⚖️ MENANG POIN - WEGO (TAK ADA LANGKAH SAH)"}
                  {winType === "resign" &&
                    "🏳️ MENANG RESIGNATION (LAWAN MENYERAH)"}
                  {winType === "timeout" &&
                    "⏳ MENANG WAKTU (JAM CATUR EXPIRED)"}
                </div>
              </div>
      ...
      <footer className="w-full text-center text-gray-500 font-serif text-[10px] sm:text-xs select-none border-t border-[#8b261a]/15 pt-4 mt-6 flex flex-col sm:flex-row justify-between items-center gap-2">
        <span>
          © 2026 MigoYugo Mahjong • Dirancang dengan penuh filosofi budi pekerti
          mahakarya timur.
        </span>
